#include "main.h"

void TIM4_IRQHandler(void)
{
	Sys_Flag ^= 1;
	//	обязательный сброс флага прерывания
	CLEAR_BIT(TIM4->SR, TIM_SR_UIF);
}

void TIM4_Init_Encoder(void)
{
	//	включаем тактирование таймера TIM4 (шина APB1)
	SET_BIT(RCC->APB1ENR, RCC_APB1ENR_TIM4EN);
	//	включаем тактирование порта B
	SET_BIT(RCC->APB2ENR, RCC_APB2ENR_IOPBEN);

	//	настройка выхода PB6
	//	сброс CNF на 00
	GPIOB->CRL &= ~GPIO_CRL_CNF6;		//	GPIO_CRL_CNF0 = 1100
	//	input floating: запись в CNF 01
	GPIOB->CRL |= GPIO_CRL_CNF6_0;		//	GPIO_CRL_CNF0_0 = 0100
	//	сброс MODE на 00
	GPIOB->CRL	&= ~GPIO_CRL_MODE6;		//	GPIO_CRL_MODE0 = 11
	//	то же самое для выхода PB7
	GPIOB->CRL &= ~GPIO_CRL_CNF7;		//	GPIO_CRL_CNF1 = 1100
	GPIOB->CRL |= GPIO_CRL_CNF7_0;		//	GPIO_CRL_CNF1_1 = 0100
	GPIOB->CRL	&= ~GPIO_CRL_MODE7;		//	GPIO_CRL_MODE1 = 11

	//	сброс флага прерывания (на всякий случай)
	CLEAR_BIT(TIM4->SR, TIM_SR_UIF);
	//	разрешаем прерывание по переполнению (перезагрузке) счетчика CNT
	//	это нужно для создания запроса на прерывание (!)
	SET_BIT(TIM4->DIER, TIM_DIER_UIE);
	//	глобально разрешаем прерывание
	NVIC_EnableIRQ (TIM4_IRQn);

	//	01: CC1 channel is configured as input, IC1 is mapped on TI1
	//	01: CC2 channel is configured as input, IC2 is mapped on TI2
	TIM4->CCMR1 |= (TIM_CCMR1_CC1S_0 | TIM_CCMR1_CC2S_0);
	TIM4->CCMR1 &= ~(TIM_CCMR1_CC1S_1 | TIM_CCMR1_CC2S_1);
	//	10: CC1 channel is configured as input, IC1 is mapped on TI2
	//	10: CC2 channel is configured as input, IC2 is mapped on TI1
//	TIM4->CCMR1 &= ~(TIM_CCMR1_CC1S_0 | TIM_CCMR1_CC2S_0);
//	TIM4->CCMR1 |= (TIM_CCMR1_CC1S_1 | TIM_CCMR1_CC2S_1);

	//	0: noninverted/rising edge
	TIM4->CCER &= ~(TIM_CCER_CC1P | TIM_CCER_CC2P);
	//	1: inverted/falling edge
//	TIM4->CCER |= (TIM_CCER_CC1P | TIM_CCER_CC2P);

	//	001: Encoder mode 1 - Counter counts up/down
	//	on TI2FP1 edge depending on TI1FP2 level
	TIM4->SMCR |= TIM_SMCR_SMS_0;
	TIM4->SMCR &= ~TIM_SMCR_SMS_1;
	TIM4->SMCR &= ~TIM_SMCR_SMS_2;
	//	010: Encoder mode 2 - Counter counts up/down
	//	on TI1FP1 edge depending on TI2FP2 level
//	TIM4->SMCR &= ~TIM_SMCR_SMS_0;
//	TIM4->SMCR |= TIM_SMCR_SMS_1;
//	TIM4->SMCR &= ~TIM_SMCR_SMS_2;
	//	011: Encoder mode 2 - Counter counts up/down
//	TIM4->SMCR |= TIM_SMCR_SMS_0;
//	TIM4->SMCR |= TIM_SMCR_SMS_1;
//	TIM4->SMCR &= ~TIM_SMCR_SMS_2;

	//	ARR register (MAX counter number)
	TIM4->ARR = 8 - 1;
	//	Counter enabled
	TIM4->CR1 |= TIM_CR1_CEN;
}
